...
@interface LSMRadioButton()
-(void)defaultInit;
-(void)otherButtonSelected:(id)sender;
-(void)handleButtonTap:(id)sender;
@end

@implementation LSMRadioButton
...
-(void)defaultInit{
    ...
    [LSMRadioButton registerInstance:self];
}
#pragma mark - Manage Instances
//添加选中的RadioButton
+(void)registerInstance:(LSMRadioButton*)radioButton{
    if(!rb_instances){
...
-(void)handleButtonTap:(id)sender{
    ...
    [LSMRadioButton buttonSelected:self];
}

#pragma mark - Class level handler
//RadioButton选中后调用的事件
+(void)buttonSelected:(LSMRadioButton*)radioButton{
    // 通知观察者
    if (rb_observers) {
        //获得观察者对象
    ...
    if (rb_instances) {
        for (int i = 0; i < [rb_instances count]; i++) {
            LSMRadioButton *button = [rb_instances objectAtIndex:i];
